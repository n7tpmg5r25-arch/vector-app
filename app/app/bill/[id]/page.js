...
export default function BillDetailPage() {
  ...
  // ER6 F10: "Report a discrepancy" affordance next to the AI disclosure.
  // Reuses the existing corrections@vectorwa.com mailbox (also on /disclaimers),
  // obfuscated the same way to keep it off naive scrapers. Prefills the bill
  // reference so a flagged summary lands with context. $0, no backend.
  const billRef = `${bill.chamber === 'House' ? 'HB' : 'SB'} ${bill.bill_number}`
  const discrepancyMailto = `mailto:${'corrections' + '@' + 'vectorwa.com'}?subject=${encodeURIComponent(`Vector | WA — possible discrepancy in ${billRef}`)}`
  ...
  // AUDIT-5 S2 (2026-07-09): the Legislation JSON-LD that Thread 82
  // injected client-side moved to the segment layout (bill/[id]/layout.js),
  // where it renders server-side so crawlers read it without executing JS.
      ...
      <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {(bill.custom_summary || bill.ai_summary) && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: bill.custom_summary ? 8 : 6, gap: 8, flexWrap: 'wrap' }}>
              <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--teal-mid)', fontWeight: 600, letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                {bill.custom_summary ? (
                  ...
                  <span
                    title="AI-drafted from the bill's official text. Confirm against the linked WA Legislature record."
                    style={{
                      fontSize: 9, padding: '2px 7px', borderRadius: 10,
            ...
            {/* ER6 F10: single consolidated method/source disclosure (above the fold).
                Replaces the duplicated "Not reviewed by a human" badge tooltip +
                disclaimer with one line framed by method + source, plus a single
                "Report a discrepancy" affordance (turns a defensive label into a
                trust/feedback loop). Only shown for unreviewed AI summaries. */}
            {!bill.custom_summary && !editingSummary && (
              <div style={{
                ...
                AI-drafted from the bill&apos;s official text, linked to the WA Legislature record above.{' '}
                <a href={discrepancyMailto} style={{ color: 'var(--text-faint)', textDecoration: 'underline' }}>Report a discrepancy</a>
                {' · '}
            ...
            {/* ER6 F10: reviewed-summary note carries the same single "Report a
                discrepancy" affordance for parity with the unreviewed disclosure. */}
            {bill.custom_summary && (
              <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid rgba(184,151,90,0.1)', fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-faint)', letterSpacing: '0.05em', lineHeight: 1.5 }}>
                AI-drafted, reviewed and edited by the operator.{' '}
                <a href={discrepancyMailto} style={{ color: 'var(--text-faint)', textDecoration: 'underline' }}>Report a discrepancy</a>
                {' · '}
