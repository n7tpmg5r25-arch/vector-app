...
- **Three-tier rule (Public / Registered / Team).** Per brand guide §08, the app has three audience tiers, all under one Vector | WA brand. Data is always free; sign-in only adds personal workflow and team collaboration on top. Defined canonically in `app/lib/viewer-capabilities.js` and enforced in `app/proxy.js`. Every thread spec MUST declare which tier(s) it touches.
  - **Public tier** (`role === 'public'`, `!user`). Anon-reachable routes: `/`, `/search`, `/bill/[id]`, `/members`, `/committees`, `/committees/[slug]`, `/hearings`, `/methodology`, `/outcomes`, `/install`, `/about`, plus `/login`, `/disclaimers` (always public). Full Vector | WA brand. No firm attribution, no upsell. Free public launch mid 2027. (Thread 71 / 2026-05-07: `/how-it-works` renamed to `/install` — old path resolves via 308 redirect in `next.config.ts`.)
...
- **Changelog standing rule:** After every PR merges, prepend an entry to `app/lib/changelog.js` as the final step of thread close-out. Group related threads into one entry — do not create one entry per thread. Each entry uses the `{ version, date, phase, highlights }` shape, newest-first, matching the file's header comment conventions. Version string must match `app/lib/version.js VERSION` at ship time.
- **Data lifecycle / retention (Thread DATA-LIFECYCLE, 2026-06-14).** Supabase free tier caps the DB at 500 MB of Postgres data (read-only beyond). Policy + markers: `app/lib/data-lifecycle.js`; full plan: `DATA_LIFECYCLE_PLAN.md` (Vector - WA folder).
  - **trajectory_snapshots + companion_snapshots** are retained by a **pg_cron job `snapshot-retention-prune`** (nightly 09:30 UTC): lossless change-point collapse (per bill keep first + last + every row whose signal changed), 7-day grace. Do NOT reintroduce daily-forever assumptions; the bill-detail chart already renders sparse change-point series, and scoreBill() does not read these tables.
  - **Fidelity tiers** hot/warm/cold by biennium age; keep the last 3 biennia re-fittable. **2027-2028 is the calibration cornerstone** (first live-captured session) — never lossy-downsample or archive it until the post-session FULL recalibration has consumed it.
  - **Archive integrity gate (never skip):** recalibrate -> materialize calibration_buckets -> re-score current session and assert final_score byte-identical before/after -> pg_dump raw -> then trim. A correct archive moves ZERO live scores.
  - Dated backup tables are not kept in the live DB (dropped 2026-06-14). `calibration_weights` (the model) + a materialized `calibration_buckets` table are the permanent baseline.
