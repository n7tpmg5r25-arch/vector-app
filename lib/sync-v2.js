/**
 * VECTOR | WA — Sync Script v2.1 (Phase 1+2 — Scoring Fix + Calibration)
 * lib/sync-v2.js
 *
 * Fetches all WA Legislature bills, scores them with the FIXED calibrated
 * trajectory model, and writes results to Supabase.
 *
 * FIXES from v1 (Phase 1):
 *  - fiscal_score: differentiates by fiscal_note_size (was always 8)
 *  - momentum_score: uses activity signals, not just days_since_action (was always 0 in interim)
 *  - sponsor_score: full 0-20 range with majority/chair/bipartisan bonuses (was 4-8)
 *  - Stage advancement bonus: +0 to +25 based on stage (raises ceiling from 60 to 99)
 *  - X Factors: positive + negative factors computed and stored (were all empty)
 *  - Confidence labels: HIGH > MODERATE > LOW > VERY LOW (were inverted)
 *  - Stalled flag: properly set when days_since_action > 21, stage <= 3, not committee_passed
 *  - substitute_filed: derived from SubstituteVersion in API response
 *  - trajectory_score: written to bills table (was always NULL)
 *
 * FIXES in v2.1 (Phase 1.5 + Phase 2):
 *  - GetRollCalls: fixed param from billId to billNumber (was returning 0 for all bills)
 *  - companion_bill: now extracted from GetLegislation Companions element
 *  - Calibration: bucket_pass_rates + category_rates updated with actual 2025-26 outcomes
 *  - Confidence labels: probabilities now match real pass rates (were 10-100x too high)
 *
 * Data flow:
...
// NEW: Get full legislation details (for companion bill)
...
async function processBill(raw, categoryRates, state) {
  ...
  const [hearings, statusChanges, sponsors, amendments, rollCalls, legislation] = await Promise.all([
    ...
    getLegislation(billNum),      // NEW: for companion bill
  ]);
...
async function runSync() {
  ...
  console.log(`[${new Date().toISOString()}] Sync v2 — ${SESSION} — state: ${state}`);
  ...
  for (let i = 0; i < allBills.length; i += BATCH) {
    ...
    process.stdout.write(`  Processing ${i}–${Math.min(i+BATCH, allBills.length)} of ${allBills.length}...\r`);
    ...
    if (i + BATCH < allBills.length) await new Promise(r => setTimeout(r, 2000));
  }
  ...
  await supabase.from('sync_log').insert({
    ...
    notes: 'sync-v2 with Phase 1 scoring fixes',
  });
