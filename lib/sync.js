...
async function getStatusChanges(billId) {
  try {
    ...
    const data = await fetchXML('GetLegislativeStatusChangesByBillId', {
      biennium:  BIENNIUM,
      billId:    billId,
...
// Get title from GetLegislation detail call
let title = '';
try {
  const detailData = await fetchXML('GetLegislation', {
    biennium: BIENNIUM, billNumber: billNum,
  });
  const leg = detailData?.ArrayOfLegislation?.Legislation;
  const detail = leg ? (Array.isArray(leg) ? leg[0] : leg) : null;
  title = detail?.LongDescription || detail?.ShortDescription ||
          detail?.LegalTitle || '';
} catch(e) { title = ''; }
